        ...
        if(!user){
            ...
            if(!passwordCompare){
                ...
                res.status(200).cookie("token", token, {
                    ...
                    domain: "https://social-media-app-g7fd.vercel.app", // Frontend domain
                    path: '/',
...
router.put("/updatepassword",fetchuser,async (req,res)=>{
    try {
        ...
        if(!oldPassword && !newPassword){
            return res.status(500).json({success:false,message:"please enter old and new password"})
        }

        const isMatch = await bcryptjs.compare(oldPassword,user.password);
        if(!isMatch){
            return res.status(500).json({success:false,message:"Old password doesn't match"})
        }
